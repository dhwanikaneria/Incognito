...
public class SearchUserComtroller extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SearchUserComtroller() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			WebResource webResource = client.resource("http://localhost:8080/RestService/getUserService/searchSharedUser");
			//MultivaluedMap formData = new MultivaluedMapImpl();
			//formData.add("username", name);
			//formData.add("password", password);
			String[] a= {username,query};
			...
			if(restResponse.getStatus()==200)
			{
				...
				RequestDispatcher rd=request.getRequestDispatcher("Friend_Reg.jsp");
